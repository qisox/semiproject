			...
			<table class="list-area" align="center" border = "1">
				...
				<div align="center">
	                <c:choose>
	                    ...
	                    <c:otherwise>
	                        <button  class='btn btn-sm' onclick="location.href='list.bo?currentPage=${pi.currentPage-1}&category=자유'">이전</button>
	                    </c:otherwise>
	                </c:choose>
	                <c:forEach var="i" begin="${pi.startPage}" end ="${pi.endPage}">
	                	<c:choose>
	                		<c:when test="${i} eq ${pi.currentPage}">
	                			<button class='btn btn-sm active' onclick="location.href='list.bo?currentPage=${i}&category=자유'">${i}</button>
	                		</c:when>
	                		<c:otherwise>
	                			<button class='btn btn-sm' onclick="location.href='list.bo?currentPage=${i}&category=자유'">${i}</button>
	                		</c:otherwise>
	                	</c:choose>
	                ...
	                <c:choose>
	                    ...
	                    <c:otherwise>
	                        <button  class='btn btn-sm' onclick="location.href='list.bo?currentPage=${pi.currentPage+1}'">다음</button>
	                    </c:otherwise>
	                </c:choose>
				</div>
				<br><br><br>
		...
		<div class="search_area">
			<form method="get" action="${contextPath }/pSearch.bo"> 
				<select id="searchCondition" name="searchCondition">
					...
					name="searchValue">
					<input type="text" name="category" value="자유" hidden>
				</span>
